...
export class DriveService {
  ...
  private async getFolderId(folderName: string, parentFolderId: string): Promise<string> {
    const response = await this.drive.files.list({
  ...
  async listFolderContents(folderName: string,
    parentFolderId: string = "1FPRL891brqHsAedXCun9_TIprnLiiBvZ"): Promise<any[]> {
    try {
        const folderId = await this.getFolderId(folderName, parentFolderId);
  ...
  async deleteFile(folderName: string, fileName: string, parentFolderId: string = "1FPRL891brqHsAedXCun9_TIprnLiiBvZ"): Promise<void> {
    try {
      const folderId = await this.getFolderId(folderName, parentFolderId);
  ...
  async renameFile(
    ...
    try {
      ...
      if (response.data.files.length === 0) {
        throw new Error(`File '${oldFileName}' not found in folder '${folderName}'`);
      }
