...
export class UserService {
  ...
  async create(createUserDto: CreateUserDto){
    ...
    return { message: 'User created.', data: savedUser, status: HttpStatus.CREATED };
  }
  ...
  async login(loginUserDto: LoginUserDto) {
    ...
    if (user) {
        ...
        if (access) {
          ...
          return { message: 'Authorized user.', data: {user, token}, status: HttpStatus.OK };
        } else {
          throw new HttpException('Unauthorized user, incorrect password.', HttpStatus.UNAUTHORIZED);
        }
    } else {
