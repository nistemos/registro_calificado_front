...
export class ProgramYearService {
    ...
    async findAll(page = 1, limit = 10, program: number) {
        const queryBuilder = this.programYearRepository.createQueryBuilder('programYear');
    
        if (program) {
            queryBuilder.where('programYear.programId = :programId', { programId: program });
        }
    ...
    async findOne(id: number) {
        const programYear = await this.programYearRepository.findOne({where:{id}});
        if (!programYear) {
            throw new HttpException(`Program year with ID ${id} not found`, HttpStatus.NOT_FOUND);
        }
        return {
            message: 'Program year found.',
            data: programYear,
    ...
    async create(createProgramYearDto: CreateProgramYearDto) {
        const existingCourse = await this.programYearRepository.findOne({ where: { name: createProgramYearDto.name, program:{id: createProgramYearDto.program}} });
        console.log("existingCourse", existingCourse)
        if (existingCourse) {
            throw new HttpException('Program year name already exists.', HttpStatus.CONFLICT);
        }

        try {
            const newProgramYear = this.programYearRepository.create(createProgramYearDto as unknown);
            const savedProgramYear = await this.programYearRepository.save(newProgramYear);
            return {
